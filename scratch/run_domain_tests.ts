...
import { RecipeSkill, LanguageEngine } from '../packages/aion-agents/src/index';

console.log('=== INICIANDO PRUEBAS DE INTEGRACIÓN DE DOMINIO AION ===\n');

const memory = AionMemoryStore.getInstance();
memory.resetToCleanState(); // Estado limpio aislado
...
// TEST 2: Cocina receta usando 300g (receta de 3 porciones = 300g total) -> inventory = 700, PreparedBatch creado
...
const recipe = {
  ...
  servings: 3, // 300g es el total para 3 porciones
...
console.log(`Resultado TEST 3: Comida registrada = "${lastMeal.preparation.name}", kcal = ${lastMeal.consumedPortion.actualKcal} (${lastMeal && lastMeal.consumedPortion.actualKcal === 200 ? 'ÉXITO' : 'FALLO'})`);

// TEST 4: Transacción compensatoria por corrección (+100g de compensación)
...
console.log(`Resultado TEST 4: Inventario recalibrado a ${inv?.amount}g (esperado 800g). Transacciones registradas: ${txs.length} (${inv?.amount === 800 && txs.length >= 2 ? 'ÉXITO' : 'FALLO'})`);
